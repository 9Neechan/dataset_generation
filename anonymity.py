...
k_counter = 7  # параметр k-anonymity, который хотим достичь
k_set_param = 0  # введите 0, если хотите рассчитать k-anonymity по исходному датасету и 1, если по обезличенному
...
def main():
    ...
    if k_set_param == 1:
        ...
        QI = ['ФИО', 'Название магазина', 'Дата и время', 'Долгота и широта', 'Номер карточки', 'Корзина', 'Количество товаров', 'Стоимость']
        k_initial, k_arr = anonymity.k_anonymity(dataset, QI)
        print(k_initial)

        # Outputting information
        print("%d размер дата-сета" % len_datadet)
        print("%d значение k-anonymity для исходного дата-сета" % k_initial)
